# regrind-color-sort - lib/palette.py
# 
...
class Palette:
	...
	def save(self, path=None):
		...
		with open(path, "w") as f:
			json.dump({"meta": self.meta, "colors": self.colors}, f, indent=2)
		self.path = path
	...
	def _ensure_cov_inv(self, c):
		if "cov" not in c:
			return
		S = np.asarray(c["cov"], np.float32)
		eps = 1e-5
		try:
			Si = np.linalg.inv(S + np.eye(3, dtype=np.float32) * eps)
		except Exception:
			Si = np.linalg.pinv(S + np.eye(3, dtype=np.float32) * eps)
		c["cov_inv"] = Si.tolist()

	def _precompute_all(self):
		for c in getattr(self, "colors", []):
			self._ensure_cov_inv(c)
